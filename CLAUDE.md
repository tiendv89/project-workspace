...
in_review → reviewing         (orchestrator, when reviewer executor is dispatched — replaces log-based reviewer_started guard)
...
- **Review-incomplete retry rule**: `review_incomplete → reviewing` uses the same first-push-wins claim protocol as reviewer dispatch. The orchestrator sets status to `reviewing` atomically before re-dispatching. After `MAX_REVIEW_INCOMPLETES` failures the orchestrator escalates directly to `blocked` instead of retrying.
...
| `reviewer_started` | orchestrator | audit only — reviewer executor dispatched; status already advanced to `reviewing` |
| `fix_started` | fix agent | audit only — fix executor dispatched; status already advanced to `in_progress` |
