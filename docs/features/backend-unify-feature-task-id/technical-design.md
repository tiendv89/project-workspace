...
- Single repo (`workflow-backend`) — per the "one task changes one repo only" rule, this fits in one task (or two same-repo tasks if the team prefers splitting `CreateWorkspaceFeature` and `insertGoTask` into separate reviewable diffs).
- Recommended task breakdown for Phase 2: one task per function change (2 tasks total), both same-repo (`workflow-backend`), with no cross-task dependency (`depends_on: []` for both) since they touch disjoint code regions and can land in either order or in parallel.
