...
## Dependency Analysis
- **New `workflow-backend` read queries** depend only on the shared Postgres schema (`handoffs`, `handoff_prs` tables) already populated by `workflow-orchestrator` — no new infra, no schema migration expected (read-only against existing tables). If `workflow-backend` does not currently have read access to these tables/schema in its own migration set, a minimal, additive (no-op for orchestrator) migration or schema-read wiring may be needed — this must be confirmed against `workflow-backend`'s existing DB bootstrap for other go-owned tables (e.g. how `insertGoTask` connects) before implementation; flagged as an open question below.
...
- **Open question for the human / task breakdown to resolve:** confirm whether `workflow-backend` already has direct Postgres access to the `handoffs`/`handoff_prs` tables (same DB as `workflow-orchestrator`) or whether a schema-sharing/migration step is needed first — resolve by inspecting `workflow-backend`'s DB config/migrations alongside `insertGoTask`'s call path before starting T1.
