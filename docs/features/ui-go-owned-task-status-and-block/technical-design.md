...
- There is **no existing backend capability for Goal 3** (recover a stuck **processing** task: `in_progress`→`ready`, `reviewing`→`in_review`, conflict `resolving`→`conflicted`). The existing `/unblock` endpoint's guard (`status = 'blocked'`) explicitly rejects any task not already `blocked`, so it cannot be reused unmodified for the processing sub-cases.

**BFF (`workflow-bff`)**
- The proxy routing (`internal/app/api/handler/proxy/routing.go`) resolves paths via longest-prefix-match against a small set of registered `bff_root_path` entries (e.g. `/api/workspaces`), not per-endpoint registration. `.../tasks/:taskId/unblock` already resolves to `workflow-backend` through the existing root-path entry, with identity headers injected generically for any path under that prefix (`TestProxyInjectsIdentityHeadersForUnblock`, `TestUnblockRouteResolvesToWorkflowBackend` cover this generic behavior, not a per-route entry). A new sibling path (`.../tasks/:taskId/recover`) falls under the same already-registered root-path prefix and resolves to `workflow-backend` automatically — **no `workflow-bff` code, config, or test change is required for this feature.**
...
- `review_incomplete` and `change_requested` must never be treated as processing states — no recover control for them.
...
### Option B — New dedicated endpoint for the stuck-processing recovery, existing `/unblock` untouched
- Add `POST /api/workspaces/:workspaceId/features/:featureId/tasks/:taskId/recover` with its own guard and its own derivation:
  ...
  - Logs a `workspace_activity_events` row with a distinct action (`recovered`) so it is auditable and distinguishable from `unblocked` in the activity feed.
...
2. **Recover stuck processing task** (new) — a new guarded endpoint in `workflow-backend`, proxied by `workflow-bff`, called by a new UI action.
...
### 2. Backend: new recover endpoint (`workflow-backend`)
- Route: `POST /api/workspaces/:workspaceId/features/:featureId/tasks/:taskId/recover` (proxied through `workflow-bff` identically to `/unblock`).
...
- Server-side guard logic (mutually exclusive, in priority order — a task can only match one, since `conflict_state='resolving'` can co-occur with `status IN {in_review, review_passed}` but never with `in_progress`/`reviewing` per the orchestrator's rebase-dispatch invariants):
  ...
  4. Else → 409 (task is not in a recoverable processing state); UI should not have shown the button, but the backend must not silently no-op.
- On success, append a `workspace_activity_events` row: `action = 'recovered'`, `note` (optional), `from`/`to` derived from the branch taken, in the same transaction as the guarded update (matching `UnblockTask`'s existing transaction pattern).
...
- `workflow-bff`: no change needed. As established in Current State above, the BFF's longest-prefix-match routing already forwards any path under the existing `/api/workspaces` root-path entry to `workflow-backend` with identity headers injected; `.../tasks/:taskId/recover` is covered automatically the same way `.../unblock` is today.
...
- **Recover action**: new button, label **"Task is stuck? Unblock it"**, rendered only when task is in a processing state as defined above (`in_progress`, `reviewing`, or `conflict_state === "resolving"`) — explicitly not shown for `blocked` or any idle/terminal status. Clicking opens a confirmation dialog with the specified warning copy and a client-computed preview of the target (same three-branch mapping as the backend, computed client-side for display only — the backend is the source of truth for what actually happens). On confirm, call the new `recover` endpoint, then refetch task detail.
...
- The recover UI action (Goal 3) is **blocked on** the new `recover` endpoint existing in `workflow-backend` (no `workflow-bff` work is needed — its generic prefix-routing already covers the new path, see Current State).
- `workflow-bff` requires **no change** for this feature — its longest-prefix-match routing already forwards the new path to `workflow-backend` automatically (see Current State / Chosen Design).
...
- **Can proceed immediately, in parallel:**
  ...
  - `workflow-backend`: new `recover` endpoint + guarded transitions — no dependency on the read-path extension (different code paths), though both touch the same `workspace_tasks` table and same files (`queries.go`, `workspace.go`, `handler/workspace.go`) so should be sequenced as two commits/PRs to avoid merge conflicts, not run as fully independent parallel workstreams within the same repo.
- **Blocked / sequenced:**
  ...
  - `digital-factory-ui` recover button (Goal 3) — blocked on the new `workflow-backend` endpoint being live (no `workflow-bff` dependency — see above).
- **Suggested sequencing for the tasks phase:** (1) `workflow-backend` read-path extension, (2) `workflow-backend` recover endpoint (no separate `workflow-bff` task needed), (3) `digital-factory-ui` all three UI changes (depends on 1–2, can be split into sub-tasks for status-display vs. the two action buttons if desired, since status-display only depends on task 1).
