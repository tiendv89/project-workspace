...
- A thin client over the BFF API — **no DB credentials**, **not published to npm** in this version. **The `workflow-mcp` repo owns its own build + global install**: `npm install && npm run build && npm link` exposes a global **`workflow-mcp`** command (via `package.json` `"bin"`), documented in its README. `install.sh` **only registers the already-built + linked server** via a command — `claude mcp add workflow-mcp --scope local --env WORKFLOW_BFF_URL=<default> --env WORKFLOW_SESSION_COOKIE=<placeholder> -- workflow-mcp`. It **assumes `workflow-mcp` is on PATH** (built + `npm link`-ed) — it does not build. The `--env` cookie/URL land in the gitignored local config (`~/.claude.json`, project-scoped); command-based, not hand-edited JSON.
...
- **T28** — `workflow-mcp` scaffold (TS, MCP TS SDK, stdio; **`package.json` `bin` + `build`; `npm link` → global `workflow-mcp`**; config from env; **agent usage docs README/AGENTS.md**; `workspace.yaml` registration)  `[workflow-mcp]` — *blocked on the new GitHub repo*
...
- **T32** — `install.sh`: **register the already-built + linked `workflow-mcp`** via **`claude mcp add … -- workflow-mcp`** (local scope; `--env` BFF URL + cookie). Assumes the `workflow-mcp` bin is on PATH (`npm link`-ed); does **not** build  `[workflow]` — *blocked on T28*
