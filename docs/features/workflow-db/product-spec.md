...
- **The TS orchestrator reads git/YAML; the Go orchestrator reads the Postgres DB.** TS keeps driving its features exactly as it does today; it may take a small additive change (e.g. declaring its `owner` to the broker) but nothing that alters legacy behaviour.
