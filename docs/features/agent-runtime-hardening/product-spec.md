...
1. Should the agent `CLAUDE.md` be written fresh each activation (overwriting any existing one) or merged with the workspace's existing `CLAUDE.md`? Leaning: written fresh to a temp location, passed via `--system-prompt` or equivalent flag, so the workspace `CLAUDE.md` is never mutated by the agent.
2. Budget enforcement via `--max-turns` is approximate (turn count ≠ token count). Is this acceptable? The original design chose the raw SDK specifically for hard token budget. Revisit: accept the approximation in v2 and add a token-count check via Claude Code's output, or retain a lightweight token tracker around the `claude` subprocess.
3. What is the exit signal? Does the runtime check task YAML state after `claude` exits (task moved to `in_review` = success) or rely purely on exit code?
