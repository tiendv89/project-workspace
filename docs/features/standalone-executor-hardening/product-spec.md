...
- **Dev infrastructure.** The dev compose stack: Redis (completion broker + dispatch queue), the broker service, the dispatch service, the orchestrator(s), and per-task executor containers. (No object store in this feature — see Non-goals.)
...
- **Take executor logs off the management repo on the standalone path.** On `local-docker` the executor **disables the git log flush** and logs to stdout (captured by the dispatcher / `docker logs`); the bundled `local-subprocess` path **retains** git logging unchanged. This removes the executor's management-repo coupling on the standalone path with **no ABI change**. Persistent log storage (S3/MinIO + per-run link) is deferred to feature `executor-log-object-storage` (same branch).
...
- **Not building persistent log storage.** The S3/object-storage log sink, the per-run log link in `result.json`, the orchestrator-side `runs[]` schema, and MinIO are feature `executor-log-object-storage` (same branch). This feature only stops the standalone executor writing logs to the management repo.
- Not changing the executor's agent-running behaviour, prompts, or `result.json` / ABI. This feature is ABI-neutral.
...
1. **Log-sink coupling — RESOLVED on the standalone path by disabling the git flush.** Today the executor flushes logs to the management *git* repo via `flushLog()` (JSONL under `docs/features/<featureId>/logs/<taskId>/`, `runtime/executors/claude/src/flush-log.ts`). New design: on `local-docker` the git flush is **off** (logs go to stdout, captured by the dispatcher); `local-subprocess` keeps it. This removes the executor's management-repo write on the standalone path with no ABI change. **Persistent log storage (S3/MinIO) is feature `executor-log-object-storage`** — until it lands, local-docker run logs are ephemeral (stdout / `docker logs` only). **In scope: the git-flush opt-out.**
...
2. **Log sink** — DECIDED: on `local-docker` the executor disables the git log flush (logs to stdout); `local-subprocess` keeps git logging. Selected by injected config (e.g. `LOG_SINK=git|none`, dispatcher sets `none`). **Persistent object-storage logging (S3/MinIO), the per-run `result.json` link, and the `runs[]` schema are deferred to feature `executor-log-object-storage`** — out of scope here.
3. **Claim location** — RESOLVED: the executor is **already claim-agnostic**. The orchestrator owns the entire claim — status → `in_progress` and the claim commit/push happen in `runtime/orchestrator/src/task/claim.ts` (`:281`, `:305-325`); the executor is spawned only if the claim is won (`main.ts:331-344`) and receives an already-claimed branch (`ExecutorPortInput`, `main.ts:405-437`). The executor writes no task status. No claim extraction needed — only the git-flush opt-out on the standalone path (item 2).
...
- On the standalone (`local-docker`) path the executor writes **no logs to the management repo** (logs to stdout); the bundled path retains git logging. The executor performs no workflow-state writes. (Persistent S3 logging is out of scope — `executor-log-object-storage`.)
- The `ExecutorPort` seam is intact: a future `K8sJobAdapter` could be added without changing the orchestrator core.
- No behavioural change to agent work, prompts, or `result.json` — this feature is ABI-neutral.
