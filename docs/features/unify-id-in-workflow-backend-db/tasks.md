...
T1 (workflow-backend: migration 00022 + reconcile + hand-written query updates)
 ...
 ├──> T3 (workspace-github-adapter: sqlc regen + UpsertWorkspaceTask/Feature + syncRunReferenceIDs)
...
## T3 — workspace-github-adapter: sqlc regen + writer updates to `id`

### Description
Update the YAML→DB sync adapter's writers to the new single-`id` schema, and fix the one
confirmed live consumer of the surrogate `id` values.

1. **`UpsertWorkspaceTask`** (`internal/database/workspace_tasks.sql.go`, generated from
...
2. **`syncRunReferenceIDs`** (`internal/worker/workspace_sync.go:438-468`) — this function
...
3. Update `sqlc.yaml`'s schema pointer if needed (it already points at
   `../workflow-backend/migrations`, so it should self-correct on `sqlc generate` — verify
   this holds after T1's migration lands).
4. Run `sqlc generate`; fix any resulting compile errors.
