...
| 8 | Task recovered *(existing — fix actor only)* | `RecoverTask` handler → `WorkspaceService.RecoverTask` → `Reader.RecoverTask` | `task` (existing, unchanged) | `recovered` (existing, unchanged) | `featureIDStr` (existing, unchanged) | `taskIDStr` (existing, unchanged) | **change**: resolved human identity instead of raw `input.Actor` UUID | **change**, branch-dependent: for the two `status`-based branches (`status 'reviewing' → 'in_review'`, `status 'in_progress' → 'ready'`) use `"Recovered from {from} → {to}"` only — no `Current status` suffix and no caller-supplied note appended, since `to` already states the resulting status unambiguously. For the `conflict_state`-based branch only (`conflict_state 'resolving' → 'conflicted'`, where the task's `status` column itself is untouched) use `"Recovered from {from} → {to}. Current status: {status}"`, where `{status}` is the task's unchanged `status` column value — the extra clarifier is needed only here because `from`/`to` describe `conflict_state`, not `status`, so without it the reader can't tell what `status` is. `{from}`/`{to}` and `{status}` all come from values `Reader.RecoverTask` already computes/returns in `RecoverTaskResult` — no new derivation needed. |
...
| 8 | Task recovered (status branch, e.g. `in_progress`→`ready`) | `task` / `recovered` / `feat-77c1` / `task-T5` / `Alice Nguyen` / `"Recovered from in_progress → ready"` |
| 8b | Task recovered (conflict branch, `resolving`→`conflicted`) | `task` / `recovered` / `feat-77c1` / `task-T6` / `Alice Nguyen` / `"Recovered from resolving → conflicted. Current status: in_progress"` |
