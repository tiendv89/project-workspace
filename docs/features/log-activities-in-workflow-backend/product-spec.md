...
| 7 | Task unblocked *(existing — fix actor only)* | `UnblockTask` handler → `WorkspaceService.UnblockTask` → `Reader.UnblockTask` | `task` (existing, unchanged) | `unblocked` (existing, unchanged) | `featureIDStr` (existing, unchanged) | `taskIDStr` (existing, unchanged) | **change**: resolved human identity instead of raw `input.Actor` UUID | **change**: `"Unblocked from status {fromStatus} → {toStatus}. Blocked reason: {blocked_reason}. Blocked details: {blocked_details}"` — `fromStatus` is always the literal `"blocked"` (the guarded precondition `Reader.UnblockTask` checks), `toStatus` is the derived resume status from `deriveUnblockTarget(blockedFromStatus)` (`"in_review"` when `blocked_from_status` was `reviewing`/`in_review`, else `"ready"`), `{blocked_reason}` is the task's stored `blocked_reason`, and `{blocked_details}` is the task's stored `blocked_details` — omit the `"Blocked details: ..."` sentence entirely when `blocked_details` is empty/null, rather than printing an empty value. Append the caller-supplied `input.Note` as a final sentence if present, rather than replacing any of the above — the transition/reason/details summary is always present, the caller note is additive. |
| 8 | Task recovered *(existing — fix actor only)* | `RecoverTask` handler → `WorkspaceService.RecoverTask` → `Reader.RecoverTask` | `task` (existing, unchanged) | `recovered` (existing, unchanged) | `featureIDStr` (existing, unchanged) | `taskIDStr` (existing, unchanged) | **change**: resolved human identity instead of raw `input.Actor` UUID | **change**: `"Recovered from {from} → {to}. Current status: {status}"` — `{from}`/`{to}` are the exact branch the guarded `UPDATE` took in `Reader.RecoverTask` (one of: `conflict_state 'resolving' → 'conflicted'`, `status 'reviewing' → 'in_review'`, or `status 'in_progress' → 'ready'` — same values the function already computes and returns in `RecoverTaskResult`, no new derivation needed), and `{status}` is the task's resulting `status` column value **after** the update (for the conflict-state branch this is the pre-existing, unchanged task status, since that branch only touches `conflict_state` and leaves `status` untouched — surfacing it makes clear to the reader that `status` did not change in that case). Append the caller-supplied `input.Note` as a final sentence if present, same additive rule as row 7. |
...
| 7 | Task unblocked | `task` / `unblocked` / `feat-77c1` / `task-T2` / `Alice Nguyen` / `"Unblocked from status blocked → ready. Blocked reason: rebase_failed. Blocked details: conflict in internal/db/migrations/0007.sql. Retried after fixing Qdrant auth."` |
| 8 | Task recovered | `task` / `recovered` / `feat-77c1` / `task-T5` / `Alice Nguyen` / `"Recovered from in_progress → ready. Current status: ready. Force-reset from stuck in_progress."` |
...
- For rows #7/#8, confirm the exact `note` phrasing/format for the from→to transition summary (this
  spec proposes `"Unblocked from status {fromStatus} → {toStatus}. Blocked reason: {blocked_reason}.
  Blocked details: {blocked_details}"` for row 7, and `"Recovered from {from} → {to}. Current status:
  {status}"` for row 8 — both with the caller-supplied note appended as a final additive sentence) —
  technical design should verify this fits the `note` column's expected length/format and any existing
  consumers, and confirm the exact omit-if-empty rule for `blocked_details` in row 7.
